...
tellraw @s [{"text":"","hoverEvent":{"action":"show_text","value":"Assumes 20 ticks per second, will be slower if the server can't keep up"}},{"text":"Estimated time remaining: "},{"score":{"name":"Hours","objective":"WorldPregen"},"color":"green"},{"text":"h"},{"score":{"name":"Minutes","objective":"WorldPregen"},"color":"green"},{"text":"m "},{"text":"[Stop]","color":"red","clickEvent":{"action":"run_command","value":"/tag @s add WorldGenStop"},"hoverEvent":{"action":"show_text","value":"Save progress and stop early"}}]
...
execute if score @s WorldPregen > Target WorldPregen run function pregen:stop
